...
| Login session cookies                                     | AES-256-GCM sealed with a key derived from `CDFIR_SESSION_SECRET`                                                                                         |
...
CDFIR_KEK_LOCAL_MASTER_KEY=<base64, exactly 32 bytes>   # openssl rand -base64 32
...
1. Generate a new key, configure both: keep the old key available under its
   key id, set `CDFIR_KEK_ACTIVE_KEY_ID=kek-2` with the new material.
...
4. Session secret rotation (`CDFIR_SESSION_SECRET`) simply invalidates active
...
`CDFIR_KEK_PROVIDER`. The row format (`kekKeyId`, wrapped DEK, IVs, tags) is

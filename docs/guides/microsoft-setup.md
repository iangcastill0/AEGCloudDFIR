...
1. Entra admin center → App registrations → New registration
   - Supported account types: *Accounts in any organizational directory and
     personal Microsoft accounts* (or narrower if you don't need personal).
...
`https://myaccount.microsoft.com` → *App permissions* invalidates collection
until reconnected.

## B. Organization mode (application permissions + admin consent)

For org-wide collection, a **tenant admin** of the *target* organization
...
3. In EvidenceVault: Connectors → Microsoft → *Organization mode* → enter the
...
- Connectors → *Test* runs a folder listing under the configured identity.

...
2. Deploy with both keys available and `CDFIR_KEK_ACTIVE_KEY_ID=kek-2`.
...
Rotate `CDFIR_SESSION_SECRET`; all sessions invalidate; no data impact.

## OIDC client secret

Create the new secret in Authentik, update `CDFIR_OIDC_CLIENT_SECRET`, deploy, delete the old one in Authentik.

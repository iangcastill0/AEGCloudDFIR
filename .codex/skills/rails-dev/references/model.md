...
A domain error is a domain concept: it lives with the model that raises it, never in a separate `app/errors/` tree. How to _handle_ errors (record vs raise, reporting, wrapping, retries) is in `references/error-handling.md`.

...
A state that carries a _when_, a _who_, or a _why_ is a record, not a boolean column. The record is a noun (`Closure`, `Publication`, `Archival`); presence is the state. Read before adding a boolean flag for business state.

...
if (!existsSync(root)) {
  console.log(
    `validate-structure: root '${root}' not found. Pass it explicitly: node scripts/validate-structure.mjs <root>`,
  );
  process.exit(0);
}
...
function listDirs(p) {
  return readdirSync(p).filter(
    (n) => !n.startsWith('.') && n !== 'node_modules' && statSync(join(p, n)).isDirectory(),
  );
}
...
function walk(dir, rootDir) {
  for (const name of readdirSync(dir)) {
    ...
    if (st.isDirectory()) {
      ...
      if (
        FORBIDDEN_DIRS.has(lower) &&
        !ALLOWED_DIRS.has(lower) &&
        !(lower === 'persistence' && underShared)
      ) {
        errors.push(
          `technical-layer folder: ${rel(full, rootDir)}/  (P11/P12 — co-locate by aggregate, use file suffixes)`,
        );
      }
      if (!ALLOWED_DIRS.has(lower) && lower !== 'persistence') {
        ...
        if (entries.length === 1 && files.length === 1) {
          warnings.push(
            `single-file folder: ${rel(full, rootDir)}/  (P14 — use a suffix instead of a folder)`,
          );
        }
      }
      walk(full, rootDir);
      ...
      if (name.toLowerCase() === 'readme.md') {
        ...
        if (segs.length > 2)
          errors.push(
            `README inside aggregate: ${rel(full, rootDir)}  (P17 — README only at package root)`,
          );
      }
      if (CODE_EXT.test(name)) {
        ...
        if (depth > MAX_DEPTH)
          errors.push(
            `depth ${depth} > ${MAX_DEPTH}: ${rel(full, rootDir)}  (P13 — flatten the layout)`,
          );
      }
    }
  }

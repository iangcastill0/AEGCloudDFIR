...
if (!existsSync(root)) {
  console.log(
    `validate-boundaries: root '${root}' not found. Pass it explicitly: node scripts/validate-boundaries.mjs <root>`,
  );
  process.exit(0);
}
  ...
  for (const name of readdirSync(dir)) {
    ...
    if (/\.(ts|tsx)$/.test(name) && !name.endsWith('.d.ts')) {
      ...
      if (name.endsWith('.entity.ts'))
        collect(/export\s+(?:abstract\s+)?class\s+([A-Za-z0-9_]+)/g, src, full, mod);
...
function checkImports(file, src, mod) {
  ...
  while ((m = re.exec(src))) {
    ...
    if (sibling && sibling !== mod && sibling !== 'shared' && sibling !== '..' && sibling !== '') {
      ...
      if (!intoBarrel)
        warnings.push(
          `cross-module deep import: ${rel(file)} -> ${spec}  (P1/P3 — import the sibling's barrel/facade only)`,
        );
    }
  }
}
...
function checkDuplicateEntities() {
  for (const [name, list] of entityNames) {
    ...
    if (mods.size > 1)
      errors.push(
        `duplicate entity name '${name}' across modules [${[...mods].join(', ')}]  (P8 — prefix with module name)`,
      );
  }
}

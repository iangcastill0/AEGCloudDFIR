...
function basicValidation(content, filename) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const char of line) {
      ...
      if (braces < 0) errors.push(`Unmatched '}' at line ${i + 1}`);
      if (brackets < 0) errors.push(`Unmatched ']' at line ${i + 1}`);
      if (parens < 0) errors.push(`Unmatched ')' at line ${i + 1}`);
    }
  }
...
async function mermaidParserValidation(content) {
  ...
  try {
    ...
    const mermaid = await import(resolve(mermaidPath, 'dist/mermaid.js')).catch(() =>
      import(resolve(mermaidPath, 'dist/mermaid.esm.mjs')).catch(() => null),
    );
...
function printResult(filename, basicResult, parserResult) {
  const hasErrors =
    basicResult.errors.length > 0 || (parserResult?.available && !parserResult?.valid);

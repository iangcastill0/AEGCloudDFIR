...
function isMmdcAvailable() {
  try {
    const mmdcPaths = [resolve(DEPS_DIR, 'node_modules/.bin/mmdc'), 'mmdc'];
...
async function renderWithBeautifulMermaid(bmPath, inputFile, outputFile, opts) {
  ...
  try {
    const bm = await import(resolve(bmPath, 'index.js')).catch(() =>
      import(resolve(bmPath, 'dist/index.js')).catch(() => import(resolve(bmPath, 'src/index.js'))),
    );

    const mmdContent = readFileSync(inputFile, 'utf-8');
    const renderFn =
      bm.renderMermaid || bm.render || bm.default?.renderMermaid || bm.default?.render;
...
function diagramNeedsIcons(content) {
  ...
  const iconPatterns = [/logos:\w/, /fa:\w/, /fa6-\w+:\w/, /mdi:\w/, /simple-icons:\w/, /aws:\w/];
...
async function main() {
  ...
  console.log(
    `Engines: mmdc=${mmdcPath ? 'yes' : 'no'}, beautiful-mermaid=${bmPath ? 'yes' : 'no'}, puppeteer=${hasPuppeteer ? 'yes' : 'no'}`,
  );
